...
ADDRESS = "localhost"
START_PORT = 20000
TCP_IP = '0.0.0.0' # this lets the os handle the selection of your ip
...
#   def __init__(self, index, address, port,server):
#       Thread.__init__(self)
#       self.index = index
#       self.server = server
#       self.sock = socket(AF_INET, SOCK_STREAM)
#       self.sock.create_connect((address, port))
#       self.valid = True
...
class Server:
  ...
  def __init__(self, pid, n, port):
    ...
    port: the port the master client wants us to listen on in which
    the master client then connects to after we spawn
    """
    self.pid = pid
    self.other_procs = [None for i in xrange(n-1)]
    self.other_procs_lock = Lock()
    # TODO: need to initialize condition variables for the logic parts
    ...
    (master_conn, (ip,client_port)) = self.master_server.accept()
    ...
    #   try:
    #     # start up the threads for communicating with all the other participants
    #     cur_handler = ClientHandler(i,ADDRESS,port_to_connect)
